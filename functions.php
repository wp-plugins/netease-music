<?php
    ...
    function get_netease_music(){
		...
		$css = nm_get_setting('small') ? 'small-cover album--nice-list' : 'album--nice-list';
		$size = nm_get_setting('small') ? '150y150' : '218y218';
        $output = '<div class="'. $css .'">';
        foreach($contents as $content){
            $index ++;
            $output .= '<div class="album--nice" data-type="163collect" data-thumbnail="'.$content['playlist_coverImgUrl'].'" data-id="'.$content['playlist_id'].'"  data-tooltip="'.$content['playlist_name'].'"><img src="'.$content['playlist_coverImgUrl'].'?param='.$size.'">
